...
## 课程介绍
本课程将带领您在已经部署 SequoiaDB 巨杉数据库引擎及安装了 SequoiaSQL-PostgreSQL 程序的环境中，创建 PostgreSQL 实例及数据库和数据表，并向数据表中写入一定量数据。
...
## 总结
通过本课程，我们通过 PostgreSQL 语法在 SequoiaSQL-PostgreSQL 实例上创建外部表与 SequoiaDB 数据库引擎的集合关联，并对数据表进行了写入和查询操作；
