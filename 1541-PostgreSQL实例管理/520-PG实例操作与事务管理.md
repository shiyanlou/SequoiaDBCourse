...
本课程将带领您在已经部署 SequoiaDB 巨杉数据库引擎（已开启事务，隔离级别为读未提交）及安装了 SequoiaSQL-PostgreSQL 程序的环境中，创建 PostgreSQL 实例及数据库和数据表，并演示事务的管理及提交与回滚。

PostgreSQL 实例事务的实现是基于 SequoiaDB 巨杉数据库存储引擎来完成的，如果需要 PostgreSQL 实例支持事务，那么 SequoiaDB 巨杉数据库存储引擎也必须开启事务，若 PostgreSQL 与 SequoiaDB 巨杉数据库的事务配置不一致时（如：PostgreSQL 的事务隔离级别为读已提交，SequoiaDB 巨杉数据库的事务隔离级别为读未提交），那么最终是以 SequoiaDB 巨杉数据库配置的为准，因为事务的最终执行是在 SequoiaDB 巨杉数据库中进行。
...
本小节将讲解如何查看并关闭 PostgreSQL 的事务功能。
...
```sql
INSERT INTO manager VALUES (1, 'Product Department') ;
```
...
6）查询写入数据，验证事务功能是否已经关闭；

```sql
SELECT * FROM manager WHERE empno = 1 ;
```

操作截图：

![图片描述](https://doc.shiyanlou.com/courses/1541/1207281/4fc298d5ea4b1c9cfef2804d43215903-0)
