...
在两个或多个 PostgreSQL 实例之间，完成元数据（如：建外部表等 DDL 操作）的同步。该工具主要用于定时解析主 SSQL 日志中新增的 DDL 操作语句并下发到各备 SSQL 执行，同时能过滤日志中执行失败的 DDL 操作语句。
...
元数据同步工具启动后会一直常驻后台运行，主要用于定时解析主 SSQL 日志中新增的 DDL 操作语句并下发到各备 SSQL 执行。

```shell
python /opt/sequoiasql/postgresql/database/5432/SsqlDisseminateDDL &
```
...
![图片描述](https://doc.shiyanlou.com/courses/1480/1207281/72dab1b5272cfe48f4ff79f44a191b96-0)
