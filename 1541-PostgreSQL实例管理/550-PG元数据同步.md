...
在两个或多个 PostgreSQL 实例之间，完成元数据（如：建外部表等 DDL 操作）的同步。该工具主要用于定时解析主 SSQL 日志中新增的 DDL 操作语句并下发到各备 SSQL 执行，同时能过滤日志中执行失败的 DDL 操作语句。本次课程以两个 PostgreSQL 实例为例（一主一备），在主 PostgreSQL 实例中创建数据库和外部表，并通过元数据同步工具，实现将新创建的数据库和外部表同步到备 PostgreSQL 实例中，完成元数据同步。
