...
db.company.employee.find () ;
```
此时可以看到 sessionA 写入的数据，即只有当 sessionA 提交数据后，sessionB 才能看到写入的数据。
...
此隔离级别下会锁定全部符合条件的纪录。 不论是读取， 还是更新， 假设 SQL 语句中包括查询条件， 则会对全部符合条件的纪录加对应的锁。
...
> transisolation参数指定隔离界别，2 表示隔离界别为读已提交。
