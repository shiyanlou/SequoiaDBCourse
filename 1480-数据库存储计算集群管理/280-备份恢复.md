...
2）本次操作恢复第二次备份的 500 条记录，所以把第二次备份的文件移动回备份目录；
...
SequoiaDB 巨杉数据库是一个分布式数据库，可以存储海量的结构化和非结构化数据，并提供多种接口实例，以方便应用程序的访问。本课程简单介绍了如何在 SequoiaDB 巨杉数据库实现全量备份，增量备份，全量恢复和增量恢复。
