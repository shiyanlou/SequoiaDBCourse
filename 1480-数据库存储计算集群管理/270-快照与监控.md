...
本课程介绍了如何通过 SequoiaDB Shell 抓取数据库快照以及如何通过 sdbtop 对数据库进行监控。
...
sdb 'db.company.createCL ( "employee", {"ShardingKey" : { "empno" : 1 } , "ShardingType" : "hash" } ) ;'
sdb 'while(true){
...
sdbtop 利用数据库的快照信息实现了对数据库集合空间、会话、系统使用自有、数据库整体信息的监控，并通过配置文件对具体的监控选项进行配置方便数据库使用人员定制自己所需的监控信息。
