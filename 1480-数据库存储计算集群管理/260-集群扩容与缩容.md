...
>Note:
>
>由于实验环境为动态生成 hostname，执行时需要把 "yourhostname" 替换为真实的值。
...
>Note:
> 
> 以上split的含义时是把 group1、group2 和 group3 上的数据分别移33%到 group4 上。
...
本课程简单介绍了如何完成巨杉数据库集群的扩容和缩容操作。在使用 split 方法对数据进行迁移时，如果一条命令迁移的数据量太大，对系统的性能是有影响的，要将影响程度降到最低，建议对数据进行分批迁移。例如：对一张大表通过一条命令迁移 50% 的数据，可分为 5 条命令，每次迁移 10% 的数据。
