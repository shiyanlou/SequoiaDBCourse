...
#### 通过 sequoiafs 启动服务
启动服务之后 mountpoint 目录下的所有文件信息会存放在远程数据库节点上，可以在数据库的目录元数据集合及文件元数据集合中查看，文件内容会以 LOB对象 的形式存放在目标集合下。通过 -i (或  --hosts) 连接指定数据库的协调节点。
...
进入到 /opt/sequoiafs/mountpoint/ 中，创建目录 fsdir 并查看。
...
进入到 /opt/sequoiafs/mountpoint/fsdir 中，创建文件 fsfile 并写入内容。
...
#### 删除文件和目录
1）以 sdbadmin 用户登录，进入到 /opt/sequoiafs/mountpoint 中，删除文件夹 fsdir：
```shell
cd /opt/sequoiafs/mountpoint
