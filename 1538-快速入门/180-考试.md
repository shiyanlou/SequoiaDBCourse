...
3) 切换至sdbadmin用户,到/opt/sequoiadb/tools/deploy目录下,使用quickDeploy.sh部署SequoiaDB；
...
11) 创建包含自增主键字段的 employee 表；

12) 使用 insert() 向 employee表中写入记录( empno : 2 , ename : "Bob", age : 29)；

13) 查询employee表；

## 挑战要求

1) 用脚本正确安装部署SequoiaDB及Mysql；
