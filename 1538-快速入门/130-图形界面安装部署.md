...
1）点击右侧工具栏"切换界面"，选择字符界面

2）解压 SequoiaDB 安装介质 sequoiadb-3.4-linux_x86_64.tar.gz，并给解压得到的 run 包增加可执行权限
```
...
10）询问是否安装 OM 服务，OM 是图形化界面服务，1 表示安装 2 表示不安装
```
----------------------------------------------------------------------------
是否安装OM服务
...
1）点击右侧工具栏的"切换界面"，选择桌面界面

2）点击桌面左上方的"Applications"，在出现的窗口中依次点击"Internet"->"Firefox web Browser"，打开FireFox 浏览器访问 "localhost:8000" 地址，登录用户密码均为 admin
> Note:
> 界面语言请选择中文
