...
#### 创建数据表
在 SequoiaSQL-MySQL 实例中创建的表将会默认使用 SequoiaDB 数据库存储引擎，包含主键或唯一键的表将会默认以唯一键作为分区键并在数据库存储引擎层自动做 hash 分区。
...
在之前的小节中通过 SequoiaSQL-MySQL 数据库实例对数据进行增删改查。对于分布式存储引擎中的数据，SequoiaDB 巨杉数据库支持通过 JSON API 的方式直接进行查询与修改。SequoiaDB 巨杉数据库存储引擎的交互式命令行界面为 SequoiaDB Shell，使用 JavaScript 作为其脚本开发语言。
...
## 巨杉数据库存储引擎事务操作
不仅可以在 SequoiaSQL-MySQL 数据库实例中进行事务控制，而且 SequoiaDB 巨杉数据库存储引擎也支持完整的事务功能：
