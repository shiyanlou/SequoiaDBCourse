...
3）切换到 SequoiaDB 快速部署脚本所在目录
```
cd /opt/sequoiadb/tools/deploy
```

4) 查看快速部署脚本的配置信息，部署1个引擎协调节点，1个编目节点与3个数据节点
```
cat sequoiadb.conf
```

5) 执行以下命令，追加两个命令至 quickDeploy.sh脚本中
```
echo "sed -i 's/service, dbPath ) ;/service, dbPath , {logfilesz:64,logfilenum:5,transactionon:true} ) ;/g'  /opt/sequoiadb/tools/deploy/quickDeploy.js" >> ./setup.sh
echo "sed -i 's/ dbPath, configure ) ;/ dbPath, {logfilesz:64,logfilenum:5,transactionon:true} ) ;/g'  /opt/sequoiadb/tools/deploy/quickDeploy.js" >> ./setup.sh
```
