...
7）提示用于安装运行数据库服务的 Linux 用户名和用户组以及密码, 输入完毕后点击"前进"。若不修改默认的用户名、用户组以及用户密码，填写确认密码，然后点击"前进"，将会创建默认的用户名（sdbadmin）和用户组（sdbadmin_group），采用默认密码 (sdbadmin)。
>Note
>
>在确认密码栏填写"sdbadmin"，确认用户密码为"sdbadmin"
...
10）在命令行终端执行命令设置 MySQL 实例安装包所属用户为 sdbadmin；
```shell
