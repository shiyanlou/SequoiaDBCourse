...
通过 mysqldumpslow 只能找出慢SQL，分析一条SQL为什么执行慢需要查看它的执行计划。在MySQL中，可以通过 explain 命令查看一条sql的执行计划。
...
开启慢查询日志后，MySQL 会记录执行时间超出 long_query_time 的 sql 语句，对于生成的慢查询日志，可以使用 MySQL 自带的 mysqldumpslow 工具进行分析。
```
