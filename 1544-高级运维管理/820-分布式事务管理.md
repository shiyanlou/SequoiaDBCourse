...
本课程主要介绍 SequoiaDB 巨杉数据库的事务功能，分别对当前实验环境 SequoiaSQL-MySQL 实例支持的 RU（读未提交）、RC（读已提交）和 RS（读稳定性）三种隔离级别进行设置和验证。由于实验环境版本为 3.4，故不在此展示 5.0 版本通过 MVCC（多版本并发控制）实现 RR（可重复读）隔离级别。
