...
本课程主要介绍 SequoiaDB 巨杉数据库的事务功能，分别对目前 SequoiaSQL-MySQL 实例支持的 RU（读未提交）、RC（读已提交）和 RS（读稳定性）三种隔离级别进行设置和验证。
...
![820-2](https://doc.shiyanlou.com/courses/1544/1207281/c25090aa4c6d74b2b3c4a1e4d07e3790-0)
...
> Note:
> - transisolation 参数指定隔离级别，0 表示隔离界别为读未提交。
...
3）再次开启新的终端（切换到 sdbadmin 用户），登录 MySQL，表示 sessionB；
...
在 SequoiaDB Shell 终端修改事务隔离级别为 RC（读已提交），并对其进行功能验证。
...
在 SequoiaDB Shell 终端修改事务隔离级别为 RS（读稳定性），并对其进行功能验证。

#### 修改事务隔离级别为 RS

1）修改事务隔离级别，2 表示隔离界别为读已提交；
...
通过本课程，我们学习了 SequoiaDB 巨杉数据库事务功能，重点学习隔离级别的设置，并通过 SequoiaSQL-MySQL 实例对目前所支持的三种隔离级别进行验证。
