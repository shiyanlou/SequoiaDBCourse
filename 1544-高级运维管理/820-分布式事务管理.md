...
开启新的终端并切换到 sdbadmin 用户下，登录 MySQL，这个终端表示 sessionA，开启事务后修改数据。
...
4）再次开启新的终端并切换到 sdbadmin 用户下，登录 MySQL，表示 sessionB；

```shell
/opt/sequoiasql/mysql/bin/mysql -h 127.0.0.1 -P 3306 -u root -proot
```

5）使用 sessionB 开启事务后查询；
