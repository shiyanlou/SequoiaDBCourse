...
本课程将带领您在已经部署 SequoiaDB 巨杉数据库引擎及创建了 MySQL 、SparkSQL 实例的环境中，通过配置协调节点的读写分离策略，实现在线联机业务与后台统计分析业务的相互隔离。其中 SequoiaSQL-SparkSQL 数据库实例包括 2 个 worker 节点，1 个 SequoiaSQL-MySQL 数据库实例节点，SequoiaDB 巨杉数据库包括 1 个引擎协调组、1 个编目组与 3 个数据组。
