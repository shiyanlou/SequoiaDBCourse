...
6) 登录到 SparkSQL 实例 Beeline Shell
...
```sql
INSERT INTO sdb.employee VALUES (10001, 'Georgi', 48), 
                            (10002, 'Bezalel', 21), 
                            (10003, 'Parto', 33), 
                            (10004, 'Chirstian', 40),
                            (10005, 'Kyoichi', 23), 
...
```sql
SELECT DISTINCT age FROM sdb.employee ORDER BY age
```

2) 查询所有人员的名字、年龄数据，并将名字作为横坐标

```sql
SELECT ename, age FROM sdb.employee ORDER BY empno
```
