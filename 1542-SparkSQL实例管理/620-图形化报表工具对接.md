...
2）Pentaho 启动内存默认设置为 2 G，在本次实验中适当调小；

```shell
sed -i "s/-Xms2048m -Xmx6144m -XX:MaxPermSize=256m/-Xms256m -Xmx512m -XX:MaxPermSize=128m/g" /opt/pentaho-server/start-pentaho.sh
```
...
4）在第一个数据源的 Query 配置项中加入查询年龄的 SQL,其它配置如图所示；
